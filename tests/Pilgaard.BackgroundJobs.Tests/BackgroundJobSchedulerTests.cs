...
public class backgroundjobscheduler_should
{
	...
	[Fact]
	public async Task not_return_duplicate_cronjobs()
	{
		// Arrange
		_services
			.AddSingleton<BackgroundJobScheduler>()
			.AddBackgroundJobs()
			// Run every 5 seconds
			.AddJob("CronJob", () => { }, CronExpression.Parse("*/5 * * * *"));

		await using var serviceProvider = _services.BuildServiceProvider();
		var sut = serviceProvider.GetRequiredService<BackgroundJobScheduler>();

		// Act
		var duplicateOccurrences = sut.GetOrderedBackgroundJobOccurrences(TimeSpan.FromHours(1))
									  .GroupBy(job => job.Occurrence)
									  .Where(group => group.Count() > 1)
									  .Select(group => group.Key);

		// Assert
		duplicateOccurrences.Should().BeEmpty();
	}
}
